...
#使用'w'创建一个csv文件并命名为f
with open(r"D:\Desktop\B站【每周必看】.csv", 'w', encoding='utf-8-sig', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['排名', '视频名', '作者', 'IP地址', '摘要', '观看网址', '观看次数', '点赞数', '收藏数', '投币数', '转发数'])
    for i, (name, _writer, ip, summary, url, view, like, collection, coin, share) in enumerate(zip(Name, Writer, IP, Summary, Url, View, Like, Collection, Coin, Share)):
