...
with open(t_addr, 'w+', encoding='utf-8-sig', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(
        ['排名', '书籍', '封面', '作者', '时间', '时间（无月日）', '出版社', '评分', '评价人数（人）', '售价（元）'])
    for i, (name, photo, _writer, time, ttime, publishment, score, people, price) in enumerate(
            zip(Name, Photo, Writer, Time, TTime, Publishment, Score, People, Price)):
