...
with open(t_addr, 'w+', encoding='utf-8-sig', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['排名', '电影', '导演', '主演', '时间', '国家', '类型', '评分', '评价人数（人）', '评语'])
    for i, (name, director, actor, time, country, type_, score, person, comment) in enumerate(
            zip(Name, Director, Actor, Time, Country, Type, Score, Person, Comment)):
        writer.writerow([i + 1, name, director, actor, time, country, type_, score, person, comment])
print('运行完成！')
